defmodule Trifolium.Plants do
  @moduledoc """
    Module to be used to interact with Trefle [Plants](https://docs.trefle.io/reference/#tag/Plants) related endpoints.
  """

  alias Trifolium.Config
  alias Trifolium.API

  @endpoint_path "api/v1/plants/"
  @http_client Config.http_client()

  @doc """
    List every possible `Plant`s.
  ...
  @doc """
    Find a specific `Plant` according to its `id` or `slug`.
  """
  ...
  @doc """
    Similar to `Trifolium.Plants.all`, but you can pass an additional `query` parameter which will full-text search
    for it in the available fields. More information can be found on Trefle documentation.
    This endpoint IS paginated, using a optional keyword parameter. By default, the page 1 is returned.
    You can use a `filter` or a `not_filter` like so:

    ```
    iex()> Trifolium.Plants.search(query, filter: %{year: year})
    ```
  ...
  @doc """
    Report a problem with a specific `Plant`. The second parameter is optional, as per Trefle docs,
